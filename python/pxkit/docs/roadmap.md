...
### Install script: autostart method selection
The base `install.sh` is implemented (git, venv, dependencies, XDG autostart,
keyring instructions). Remaining work:

- Detect whether systemd is available (`systemctl --version`)
- If yes: prompt user to choose XDG autostart or systemd user service
- If no: use XDG autostart silently, no prompt
- Write chosen method to `autostart:` key in `pxkit.yaml` (supports future
  uninstall path)
- systemd path: write `~/.config/systemd/user/pxkit.service`, run
  `systemctl --user daemon-reload && systemctl --user enable pxkit`
- XDG path: write `~/.config/autostart/pxkit.desktop` (current behavior)
